...
async function getMimeContentOfMessageAsync(messageId) {
    // Ensure client isn't undefined
    if (!_userClient) {
        throw new Error('Graph has not been initialized for user auth');
    }

    return _userClient.api(`/me/messages/${messageId}/$value`)
        .get();
}

module.exports.getMimeContentOfMessageAsync = getMimeContentOfMessageAsync;
